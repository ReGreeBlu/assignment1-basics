...
vocab_path = "output/tinystories_valid_vocab.json"
merges_path = "output/tinystories_valid_merges.json"
special_tokens = ["<|endoftext|>"]
tokenizer_ts_valid = Tokenizer.from_files(vocab_path, merges_path, special_tokens)

vocab_path = "output/owt_valid_vocab.json"
merges_path = "output/owt_valid_merges.json"
special_tokens = []
tokenizer_owt_valid = Tokenizer.from_files(vocab_path, merges_path, special_tokens)

vocab_path = "output/tinystories_train_vocab.json"
merges_path = "output/tinystories_train_merges.json"
special_tokens = ["<|endoftext|>"]
tokenizer_ts_train = Tokenizer.from_files(vocab_path, merges_path, special_tokens)
...
for text in sample_text:
    ...
    token_count += len(tokenizer_ts_valid.encode(text))
...
for text in sample_text:
    bytes_count += len(text.encode("utf-8"))
    token_count += len(tokenizer_owt_valid.encode(text))
...
for text in sample_text:
    tokenizer_owt_valid.encode(text)
...
input_path = "data/TinyStoriesV2-GPT4-valid.txt"
with open(input_path, "r") as f:
    total_ids = list(tokenizer_ts_valid.encode_iterable(f))
...
input_path = "data/owt_valid.txt"
with open(input_path, "r") as f:
    total_ids = list(tokenizer_owt_valid.encode_iterable(f))
arr = np.array(total_ids, dtype=np.uint16)
...
with open(input_path, "r") as f:
    total_ids = list(tokenizer_ts_train.encode_iterable(f))
arr = np.array(total_ids, dtype=np.uint16)
